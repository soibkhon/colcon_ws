...
def generate_launch_description():
    ...
    # Use the modified params file without AMCL
    default_nav_params = os.path.join(pkg_wheelchair_core, 'config', 'nav2_params_map_rls_backup.yaml')
    ...
    # CRITICAL: Static transform from map to odom (replaces AMCL)
    # This sets the initial position of the robot in the map
    ...
    # Transform from base_link to livox_frame
    static_transform_livox_cmd = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='static_transform_livox',
        arguments=[
            '--x', '0.54', '--y', '0', '--z', '0.28',  # Adjust LiDAR position
    ...
    # REMOVED: AMCL node - no longer needed
    # REMOVED: lifecycle_manager_localization - no longer needed
    
    # Lifecycle manager for map_server only (navigation lifecycle is in nav2_bringup)
    lifecycle_manager_localization = Node(
        ...
        parameters=[{
            'use_sim_time': use_sim_time,
            'autostart': True,
            'node_names': ['map_server']  # Only map_server, no AMCL
        }]
    )
    ...
    return LaunchDescription([
        ...
        static_transform_map_to_odom,      # NEW: map->odom transform
        static_transform_livox_cmd,
        static_transform_base_to_base,
        static_transform_base_to_root,     # Added this back
        map_server_node,
        lifecycle_manager_localization,    # Modified: only map_server
        nav2_bringup_group,
        rviz_node,
    ])